...
class ShallowNetwork(Network):
    ...
    def __init__(self, action_space: dict, state_size: int, hidden_units: int, learning_rate: float):
    ...
    def initialize(self):
        ...
        with self.graph.as_default():
            self.state = tf.placeholder(tf.float32, [1, 1], name='state')
            #reward = tf.placeholder(tf.float32, None, name='reward')
            #next_state = tf.placeholder(tf.float32, [None, self.state_size], name='next_state')
            # Replace Q by reward 
            state_size = tf.placeholder(tf.int32)
            W = tf.Variable(self.initializer(shape=[state_size, self.hidden_units * self.num_actions]),
                            name='common_weights', validate_shape=False)
            ...
            for key, sub_space in self.action_space.items():
                ...
                for sub_space_key, sub_space_value in sub_space.items():
                    ...
                    weights[key][sub_space_key] = tf.Variable(
                        self.initializer([int(common_layer.shape[1]), max(sub_space_value)]),
